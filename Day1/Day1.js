...
// Setting a counterer
var counter = 0;
// Setting an index counter, will be used to return doubled frequency
var i = 0;

// While we don't have the frequency appearing twice
while(counter === 0){
    // Mapping on puzzle
    puzzle.map(x => {
        // Getting frequency
        frequency += parseInt(x, 10);
        // Pushing frequency in array
        array.push(frequency);
        // Mapping frequencies array to search if frequency is existing in and counter
        array.map(v => (parseInt(v, 10) === frequency && counter++));
        // If frequency appears twice, counter will be 2
        if (counter <= 1) {
            counter = 0;
        } else {
            // Returning counter to break loop
            return counter;
        }
        i++;
    });
